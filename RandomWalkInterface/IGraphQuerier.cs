...
namespace RandomWalkFramework.RandomWalkInterface {
	...
	public interface IGraphQuerier<TVertex> : IDisposable {
		/// <summary>
		...
		IEnumerable<TVertex> AdjecentEdges(TVertex vertex);
		/// <param name="vertex">The state to be queried</param>
		...
		TVertex AdjecentEdge(TVertex vertex, int index);
		...
		/// </example>
		TVertex WeightedAdjacentEdge(TVertex vertex, decimal weightedIndex);
		...
		decimal EdgeWeight(TVertex source, TVertex target);

		decimal EdgeWeightAt(TVertex source, int adjacentIndex);
