...
namespace RandomWalkFramework.Querier
{
    ...
    public class GeneralWeightedQuerier<TVertex> : UnweightedGraphQuerier<TVertex> {

      
		protected WeightFunction<TVertex> wf;
		...
		public GeneralWeightedQuerier(IAdjacencyGraph<TVertex> targetGraph, WeightFunction<TVertex> wf, KeyValuePair<string, string> policyName, int degreeThreshold)
		...
		protected class WeightedEdgeMapping {
			...
			//Dictionary<TVertex, decimal> weightsMap = new Dictionary<TVertex, decimal>();
			GeneralWeightedQuerier<TVertex> parent;
			
			bool IsDirected;

			public WeightedEdgeMapping(GeneralWeightedQuerier<TVertex> parent, bool IsDirected, TVertex vertex, IList<TVertex> adjecentEdges, WeightFunction<TVertex> weightFunc) {
				this.vertex = vertex;
				this.parent = parent;
				...
				for (int i = 0; i < adjecentEdges.Count; i++) {
					map.Add(cw);	
					parent.TryGetEdgeWeight(vertex, adjecentEdges[i], out nw);					
					cw += nw;						
				}
				VertexWeight = cw;
			...
			public decimal GetWeightAt(int index) {
				decimal nextw = (index+1 == map.Count?VertexWeight:map[index+1]), currw = map[index];
				return nextw - currw;
		...
		public override TVertex WeightedAdjacentEdge(TVertex vertex, decimal weightedIndex) {
		...
		public bool TryGetEdgeWeight(TVertex source, TVertex target, out decimal weight) {
			weight = wf(targetGraph, source, target);
			return false;
		}

		public override decimal EdgeWeightAt(TVertex source, int adjacentIndex) {
			WeightedEdgeMapping wem;
			wem = VertexMapping(source);
			return wem.GetWeightAt(adjacentIndex);
		}

		public override decimal EdgeWeight(TVertex source, TVertex target) {
			return EdgeWeightAt(source, targetGraph.AdjacentEdgeIndex(source, target));
		}
		#endregion
		...
		protected virtual WeightedEdgeMapping VertexMapping(TVertex vertex) {
			...
			try {
				...
				wem = new WeightedEdgeMapping(this, targetGraph.IsDirected, vertex, edgelist, wf);				
